...
{
	tapis_Marche = self.Bool;
	self.Bool++;
	tapis_Deplacement = self.Float;
	self.Float++;
}
...
void variablesInitialization(void)
{
	...
	CB_Char_Init(Mem, 0 + 3, 't');
	CB_Char_Init(Mem, 0 + 4, 'a');
	CB_Char_Init(Mem, 0 + 5, 'p');
	CB_Char_Init(Mem, 0 + 6, 'i');
	CB_Char_Init(Mem, 0 + 7, 's');
	CB_Char_Init(Mem, 0 + 8, '.');
	CB_Char_Init(Mem, 0 + 9, 'e');
	CB_Char_Init(Mem, 0 + 10, 'x');
	CB_Char_Init(Mem, 0 + 11, 'e');
